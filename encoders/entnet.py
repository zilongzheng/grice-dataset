...
class EntNet(nn.Module):
    def __init__(self, config, vocabulary):
        ...
        self.story_enc = InputEncoder(config["max_seq_len"] * 2, self.embed_size, self.device)
        self.query_enc = InputEncoder(config["max_seq_len"] * 2, self.embed_size, self.device)

        self.cell = MemCell(self.num_blocks, self.embed_size, self.prelu, self.device)
        self.output = OutputModule(self.num_blocks, config["rnn_hidden_size"], self.embed_size, self.prelu, self.device)
    ...
    def forward(self, batch):
        ...
        batch_size, max_seq_len = ctx_ques_ans.size()
        # ctx_query  = torch.cat([ctx_ques_ans, ctx_ans], dim=-1)
        ctx_query = ctx_ques_ans.view(batch_size, max_seq_len)

        num_rounds = ctx_hist.size(1)
        ctx_hist = ctx_hist.view(batch_size * num_rounds, max_seq_len)

        hist_embeded = self.word_embed(ctx_hist)
        query_embedded = self.word_embed(ctx_query)

        # (batch_size  * num_rounds, max_seq_len * 2)
        hist_embeded = self.story_enc(hist_embeded.view(batch_size, num_rounds, max_seq_len, self.embed_size))
