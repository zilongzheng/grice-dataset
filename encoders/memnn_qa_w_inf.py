...
class MemNNQA(nn.Module):
    def __init__(self, config, vocabulary, inference_encoder=None):
        super().__init__()
        ...
        self.fusion = nn.Linear(fusion_size, self.vocab_size)
    ...
    def forward(self, batch):
        ...
        # print(self.vocabulary.to_words(ctx_ans[0][0].cpu().data.numpy()))
        # for i in range(10):
        ...
        num_rounds = ctx_ques.size(1)

        inf_exp_ans = self.inference_encoder(batch)
        inf_exp_ans = inf_exp_ans.view(batch_size * num_rounds, self.rnn_hidden_size)
        ...
        ans_embed = torch.concat([ans_embed, inf_exp_ans])
        ans_embed = self.answer_fusion(ans_embed)
        ans_embed = self.dropout(ans_embed)
        ...
        hist_embed = self.context_fusion(torch.cat([ques_embed, ans_embed], dim=-1))
        hist_embed = self.dropout(hist_embed)
        ...
        hist_att = F.softmax(hist_att, dim=-1)
        ...
        fused_embedding = fused_embedding.view(batch_size, num_qa, self.vocab_size)
